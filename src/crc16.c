...
uint16_t CRC16_CCITT_InitCRC(void) EMBETILE(CRC16_CCITT_InitCRC);
uint16_t CRC16_CCITT_UpdateCRC_Byte(uint8_t byte, uint16_t crc) EMBETILE(CRC16_CCITT_UpdateCRC_Byte);
uint16_t CRC16_CCITT_UpdateCRC_Buf(const void* buf, size_t size, uint16_t crc) EMBETILE(CRC16_CCITT_UpdateCRC_Buf);
int      CRC16_CCITT_CheckCRC(const void* buf, size_t size, uint16_t crc) EMBETILE(CRC16_CCITT_CheckCRC);
uint16_t CRC16_MODBUS_InitCRC(void) EMBETILE(CRC16_MODBUS_InitCRC);
uint16_t CRC16_MODBUS_UpdateCRC_Byte(uint8_t byte, uint16_t crc) EMBETILE(CRC16_MODBUS_UpdateCRC_Byte);
uint16_t CRC16_MODBUS_UpdateCRC_Buf(const void* buf, size_t size, uint16_t crc) EMBETILE(CRC16_MODBUS_UpdateCRC_Buf);
int      CRC16_MODBUS_CheckCRC(const void* buf, size_t size, uint16_t crc) EMBETILE(CRC16_MODBUS_CheckCRC);
#endif

#if defined     CRC16_CCITT_USE_LUT && (CRC16_CCITT_USE_LUT == 1)
static uint16_t CRC16_CCITTLUT[256] = {0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7, 0x1081, 0x0108, 0x3393, 0x221a,
...
static uint16_t CRC16_MODBUSLUT[] = {0X0000, 0XC0C1, 0XC181, 0X0140, 0XC301, 0X03C0, 0X0280, 0XC241, 0XC601, 0X06C0, 0X0780, 0XC741, 0X0500, 0XC5C1, 0XC481, 0X0440, 0XCC01, 0X0CC0, 0X0D80, 0XCD41,
...
uint16_t CRC16_CCITT_UpdateCRC_Buf(const void* buf, size_t size, uint16_t crc) {
    ...
    for (i = 0; i < size; i++) {
        crc = CRC16_CCITT_UpdateCRC_Byte(((const uint8_t*)buf)[i], crc);
    }
    return crc;
}
...
uint16_t CRC16_MODBUS_UpdateCRC_Buf(const void* buf, size_t size, uint16_t crc) {
    ...
    for (i = 0; i < size; i++) {
        crc = CRC16_MODBUS_UpdateCRC_Byte(((const uint8_t*)buf)[i], crc);
    }
    return crc;
}
...
int CRC16_CCITT_CheckCRC(const void* buf, size_t size, uint16_t crc) {
    ...
    // initialize CRC
    buf_crc = CRC16_CCITT_InitCRC();

    // calculate CRC of the input buffer
    buf_crc = CRC16_CCITT_UpdateCRC_Buf(buf, size, buf_crc);

    // compare values
    if ((uint16_t)buf_crc == (uint16_t)crc) {
        // CRC match
        return 1;
    }
...
int CRC16_MODBUS_CheckCRC(const void* buf, size_t size, uint16_t crc) {
    ...
    // initialize CRC
    buf_crc = CRC16_MODBUS_InitCRC();

    // calculate CRC of the input buffer
    buf_crc = CRC16_MODBUS_UpdateCRC_Buf(buf, size, buf_crc);
