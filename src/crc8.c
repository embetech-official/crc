/**
 * @file
 * @license   ${PROJECT_LICENSE}
 * @copyright ${PROJECT_COPYRIGHT}
 * @version   ${PROJECT_VERSION}
 * @purpose   8-bit CRC calculation routines
 * @brief     8-bit CRC calculation routines
 */

#include "embetech/crc8.h"

uint8_t CRC8_ITU_Init(void) { return 0; }

uint8_t CRC8_ITU_UpdateLut(void const *buf, size_t size, uint8_t crc) {
  static uint8_t const crc8ItuLut[256] = {
      };

  uint8_t const *data = (uint8_t const *)buf;
  for(size_t i = 0; i < size; ++i) {
    crc = crc8ItuLut[crc ^ data[i]];
  }

  return crc;
}

uint8_t CRC8_ITU_UpdateFormulaIc(void const *buf, size_t size, uint8_t crc) {
  uint8_t const *data = (uint8_t const *)buf;
  for(size_t i = 0; i < size; ++i) {
    crc ^= data[i];
    for(uint8_t j = 0; j < 8; ++j) {
      if(crc & 0x80)
        crc = (crc << 1) ^ 0x07;
      else
        crc <<= 1;
    }
  }
  return crc;
}

uint8_t CRC8_ITU_Finalize(uint8_t crc) { return crc ^ 0x55; }
