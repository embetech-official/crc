...
uint8_t CRC8_ITU_UpdateUsingLut(void const *data, size_t data_size, uint8_t crc) {
  ...
  uint8_t const *byte = (uint8_t const *)data;
  for(size_t i = 0; i < data_size; ++i) {
    crc = crc8ItuLut[crc ^ byte[i]];
  }
...
uint8_t CRC8_ITU_UpdateUsingFormula(void const *data, size_t data_size, uint8_t crc) {
  ...
  for(size_t i = 0; i < data_size; ++i) {
    ...
    for(uint8_t j = 0; j < 8; ++j) {
      crc = ((crc & 0x80) != 0) ? (uint8_t)((crc << 1) ^ 0x07) : (uint8_t)(crc << 1);
    }
  }
  return crc;
}

uint8_t CRC8_ITU_Finalize(uint8_t crc) { return (uint8_t)(crc ^ 0x55U); }
